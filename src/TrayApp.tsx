...
export default function TrayApp() {
    ...
    return (
        <div className="h-screen w-full bg-[#141415]/90 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col p-4 text-white overflow-hidden shadow-2xl items-center relative select-none">
            ...
            {/* Mode Switcher */}
            {!isBusy && (
                <div className="flex bg-zinc-900/80 p-1 rounded-full mb-6 w-full border border-white/5 relative">
                    <div
            ...
            <AnimatePresence mode="wait">
                {mode === 'broadcast' ? (
                    <motion.div key="broadcast" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col items-center w-full flex-1">
                        {!isBroadcasting ? (
                            <div className="flex flex-col items-center justify-center h-full gap-4 mt-4">
                                <div className="text-zinc-400 text-sm text-center">Ready to cast</div>
                                <button
                            ...
                            <div className="flex flex-col items-center justify-center h-full gap-4 mt-4">
                                <div onClick={() => host.stopHost()} className="relative group cursor-pointer">
                    ...
                    <motion.div key="listen" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col items-center w-full flex-1">
                        {!isListening ? (
                            <div className="w-full h-full flex flex-col pt-2">
                                <div className="flex justify-between items-center mb-3">
                                    <span className="text-xs font-semibold text-zinc-500 uppercase">Available</span>
                                    <button onClick={() => receiver.discoverHosts()} className="text-zinc-400 hover:text-white"><RefreshCw size={14} /></button>
                                </div>
                                <div className="space-y-2 flex-1 overflow-y-auto w-full no-scrollbar">
                                    {receiver.hosts.length === 0 ? (
                                        <div className="py-8 text-center text-xs text-zinc-500">No broadcasts found</div>
                                    ) : (
                                        receiver.hosts.map((hostNode: any, i: number) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className="w-full flex items-center p-3 rounded-xl bg-zinc-900/50 hover:bg-white/5 border border-white/5 hover:border-white/20 transition-all text-left"
                                                >
                                                    ...
                                                    <div className="flex-1 truncate">
                                                        <div className="font-medium text-xs text-white truncate">{hostNode.name}</div>
                            ...
                            <div className="flex flex-col items-center justify-center h-full gap-4 mt-4 w-full">
